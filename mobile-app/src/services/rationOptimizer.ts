...
/*  COPY of /home/z/my-project/src/lib/feed-lp.ts, adapted for RN.    */
/*  No React imports; pure TypeScript — runs entirely on-device.       */
/* ================================================================== */
 ...
 * Returns infeasible when the LP cannot be satisfied.
 */
...
export interface FormulateParams {
  animalKey: AnimalKey;
  weight: number;
  production: number;
  prices: PriceMap;
...
export function formulateRation(params: FormulateParams): FormulationResult {
  ...
  const cost = INGREDIENT_ORDER.map(
    (k) => params.prices[k] ?? INGREDIENTS[k].defaultPrice
  );
  ...
  // 5) Roughage >= forageMin (ruminants only)
  ...
  // 6) Per-ingredient lower bounds + upper bounds
  const upperBounds: number[] = new Array(nVars).fill(1);
  INGREDIENT_ORDER.forEach((k, i) => {
    const bnd = animal.bounds[k];
    if (bnd.lb > 0) {
      const coeff = new Array(nVars).fill(0);
      coeff[i] = 1;
      constraints.push({ coeff, op: ">=", rhs: bnd.lb / 100 });
    }
    upperBounds[i] = bnd.ub / 100;
  });
  ...
  const components = INGREDIENT_ORDER.map((k, i) => {
    ...
    const c = +(
      result.x[i] *
      dmi *
      (params.prices[k] ?? INGREDIENTS[k].defaultPrice)
    ).toFixed(2);
  ...
  const achieved = {
    cp: INGREDIENT_ORDER.reduce(
      (s, k, i) => s + result.x[i] * INGREDIENTS[k].protein,
      0
    ),
    tdn: INGREDIENT_ORDER.reduce(
      (s, k, i) => s + result.x[i] * INGREDIENTS[k].tdn,
      0
    ),
    fiber: INGREDIENT_ORDER.reduce(
      (s, k, i) => s + result.x[i] * INGREDIENTS[k].fiber,
      0
    ),
  };
  ...
  if (achieved.cp < cpMin - 0.3)
    warnings.push("البروتين أقل قليلاً من الهدف — راجع حدود الصويا.");
  if (achieved.tdn < tdnMin - 0.5)
    warnings.push("الطاقة أقل من الهدف — اسمح بمزيد من الذرة.");
  if (achieved.fiber > fiberMax + 0.5)
    warnings.push("الألياف أعلى من الموصى — قلّل التبن/الدريس.");
...
export function computeManualResult(
  ...
  const components = INGREDIENT_ORDER.filter(
    (k) => (percents[k] ?? 0) > 0.05
  ).map((k) => {
  ...
  const sumPct = INGREDIENT_ORDER.reduce(
    (s, k) => s + (percents[k] ?? 0),
    0
  );
